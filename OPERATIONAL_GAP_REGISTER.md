...
| GAP-031 | `Document` không có trường xác định người phê duyệt (approver) — `Document::$fillable` (`app/Models/Document.php:96-126`) không chứa `approved_by`/`approved_at`/`approval_note`/tương đương cho reject. `DocumentController::approve()` (`app/Http/Controllers/Web/DocumentController.php:209-230`) vẫn gọi `$document->update(['approved_by' => Auth::id(), 'approved_at' => now(), 'approval_note' => ...])` — các key này bị Eloquent mass-assignment **âm thầm bỏ qua** vì không nằm trong `$fillable` (không throw, không lỗi) → dữ liệu audit "ai duyệt, lúc nào, ghi chú gì" không được ghi nhận thật dù `status` đổi thành `approved` thành công. Đồng thời không có cột `approver_id`/tương đương nào trên `Document` để xác định trước "tài liệu này cần đúng người X duyệt" — `DocumentController::approvals()` chỉ lọc theo tenant + `is_active` + tham số tuỳ chọn (`project_id`/`status`), không theo approver cụ thể. | **OPEN (verified)** | `app/Models/Document.php:96-126` (fillable list, không có field approver); `app/Http/Controllers/Web/DocumentController.php:209-230` (approve ghi field không fillable), `:175-193` (approvals() không lọc theo approver); phát hiện khi thiết kế `docs/superpowers/specs/2026-07-31-today-workspace-mvp-design.md` (Today Workspace "Action Required" — Document approval bị loại khỏi nguồn vì lý do này, xem mục 6.4/7 của spec đó) | Cùng dạng lỗi "cập nhật im lặng bị bỏ qua" như GAP-002 (Tier 1) nhưng ở quy mô 3 field audit của 1 hành động phê duyệt, không phải toàn bộ record. Đề xuất sửa (không thực hiện trong lần phát hiện này): (1) thêm `approved_by`/`approved_at`/`approval_note`/`rejected_by`/`rejected_at`/`rejection_note` vào `$fillable` (hoặc dùng `$guarded` hẹp hơn), có migration nếu cột DB chưa tồn tại — cần xác nhận trước; (2) nếu muốn Document tham gia được vào 1 luồng "cần tôi duyệt" cá nhân hoá (VD cho Today Workspace sau này), cần thêm cơ chế xác định approver xác định trước (cột `approver_id` hoặc bảng phê duyệt riêng), không suy đoán ai là approver từ permission `document.approve` toàn tenant. Ghi nhận thêm: `DocumentController::approve()` gọi `Document::findOrFail($documentId)` không có `->where('tenant_id', ...)` tường minh trong câu lệnh, dựa hoàn toàn vào global scope `TenantScope` (`Document.php:38`) để cách ly tenant — không kết luận có lỗ hổng (scope có áp dụng), nhưng đáng verify riêng nếu có ai sửa lại luồng này. |
...
| GAP-029 | Operator web UI chưa có đường "mở lại để sửa"/nộp lại (`revising` → resubmit) cho Submittal bị reject — backend/API đã đủ, UI chưa nối | **OPEN (verified)** | `resources/views/submittals/show.blade.php:93` chỉ có nhánh `status === 'draft'`, không có nhánh `rejected`/`revising`; `app/Http/Controllers/Web/SubmittalPageController.php:122-168` có `submit/approve/reject` nhưng không có `startRevision`, không proxy `POST /submittals/{id}/start-revision` (route đã có ở API, xem `routes/api_zena.php`) | PR#229 (2026-07-24/25) đóng ngõ cụt state machine ở tầng API (`SubmittalLifecycleService`, bảng `submittal_revisions`, `SubmittalPolicy`) nhưng CỐ Ý không nối UI — quyết định scope từ lúc brainstorm, final whole-branch review xác nhận lại đúng dự đoán. Người dùng thật vẫn phải gọi API trực tiếp mới resubmit được. Đề xuất: thêm nút "Sửa lại" ở `show.blade.php` khi `status===rejected` → gọi `POST .../start-revision`, mở form edit + field `revision_summary` bắt buộc khi resubmit, theo đúng contract đã có ở `SubmittalController::startRevision()`/`submit()`. Liên quan GAP-013 (cùng entity Submittal, khác khía cạnh — GAP-013 là thiếu thông báo, GAP-029 là thiếu lối vào UI). |
