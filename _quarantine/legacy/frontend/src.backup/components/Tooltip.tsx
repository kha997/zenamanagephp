...
const Tooltip: React.FC<TooltipProps> = ({
  ...
  const showTooltip = () => {
    ...
    timeoutRef.current = setTimeout(() => {
      setIsVisible(true)
      // Position will be calculated in useEffect after tooltip is rendered
    }, delay)
  }
  ...
  useEffect(() => {
    if (isVisible) {
      ...
      // Use requestIdleCallback for resize/scroll handlers to avoid blocking main thread
      ...
      return () => {
        ...
        if (typeof resizeTimeout === 'number') {
          if ('cancelIdleCallback' in window) {
            (window as any).cancelIdleCallback(resizeTimeout);
          } else {
            clearTimeout(resizeTimeout as NodeJS.Timeout);
          }
        }
        if (typeof scrollTimeout === 'number') {
          if ('cancelIdleCallback' in window) {
            (window as any).cancelIdleCallback(scrollTimeout);
          } else {
            clearTimeout(scrollTimeout as NodeJS.Timeout);
          }
        }
      ...
      {isVisible && (
        <motion.div
          ...
          transition={{ 
            duration: 0.15, 
            ease: 'easeOut',
            // Use transform instead of opacity/scale for better performance
            type: 'tween'
          }}
          className={cn(
          ...
          style={{
            left: `${position.x}px`,
            top: `${position.y}px`,
            // Use GPU acceleration and avoid reflow
