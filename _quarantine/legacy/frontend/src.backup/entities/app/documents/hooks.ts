...
export const documentsKeys = {
  ...
  kpis: (period?: string) => [...documentsKeys.all, 'kpis', period] as const,
  alerts: () => [...documentsKeys.all, 'alerts'] as const,
  activity: (limit: number) => [...documentsKeys.all, 'activity', limit] as const,
};
...
// KPI, Alert, and Activity hooks for Universal Page Frame
export const useDocumentsKpis = (period?: string) => {
  return useQuery({
    queryKey: documentsKeys.kpis(period),
    queryFn: () => documentsApi.getDocumentsKpis(period),
    staleTime: 60_000,
    retry: 1,
...
export const useDocumentsAlerts = () => {
  return useQuery({
    queryKey: documentsKeys.alerts(),
...
export const useDocumentsActivity = (limit: number = 10) => {
  return useQuery({
    queryKey: documentsKeys.activity(limit),
