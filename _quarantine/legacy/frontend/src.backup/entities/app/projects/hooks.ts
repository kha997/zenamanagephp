...
export const projectsKeys = {
  ...
  kpis: (period?: string) => [...projectsKeys.all, 'kpis', period] as const,
  alerts: () => [...projectsKeys.all, 'alerts'] as const,
  activity: (limit: number) => [...projectsKeys.all, 'activity', limit] as const,
};
...
export const useProjectsKpis = (period?: string) => {
  return useQuery({
    queryKey: projectsKeys.kpis(period),
    queryFn: () => projectsApi.getProjectsKpis(period),
    staleTime: 60_000, // 1 minute
    retry: 1,
...
export const useProjectsAlerts = () => {
  return useQuery({
    queryKey: projectsKeys.alerts(),
    queryFn: () => projectsApi.getProjectsAlerts(),
    staleTime: 60_000, // 1 minute
    retry: 1,
    refetchOnWindowFocus: false,
    refetchOnMount: false,
...
export const useProjectsActivity = (limit: number = 10) => {
  return useQuery({
    queryKey: projectsKeys.activity(limit),
    queryFn: () => projectsApi.getProjectsActivity(limit),
    staleTime: 60_000, // 1 minute
    retry: 1,
    refetchOnWindowFocus: false,
    refetchOnMount: false,
