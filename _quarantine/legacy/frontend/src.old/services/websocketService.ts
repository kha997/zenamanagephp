...
class WebSocketService {
  ...
  private connect() {
    ...
    if (!token) {
      // Silently skip if no auth token
      return
    }

    // Temporarily disable WebSocket connection until server is ready
    // Commented out to reduce console noise - will enable when WebSocket server is ready
    // if (import.meta.env.DEV) {
    //   console.debug('WebSocket connection disabled - server not ready yet')
    // }
    return
