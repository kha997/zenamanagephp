    ...
    (set, get) => ({
      ...
      clearAuth: () => {
        ...
        set({
          ...
          error: null,
          isLoading: false,
          isInitialized: true, // Keep initialized true so queries can run and detect auth failure
        });
      },
      ...
      refreshUser: async () => {
        ...
        try {
          ...
          // If token is invalid, try to get new token from session
          if (error instanceof Error && 'response' in error) {
            const axiosError = error as { response?: { status?: number } };
            if (axiosError.response?.status === 401 || axiosError.response?.status === 403) {
              // Token might be invalid, try to reinitialize auth
...
// If no token but user is authenticated via web session, fetch token from session
export const initializeAuth = async () => {
  const token = localStorage.getItem('auth_token');
  if (token) {
    apiClient.defaults.headers.common['Authorization'] = `Bearer ${token}`;
    useAuthStore.setState({ token, isAuthenticated: true, isInitialized: true });
    return;
  }

  // Try to get token from web session if user is authenticated
  try {
    ...
    if (response.ok) {
      ...
      if (data.success && data.token) {
        ...
        // No token available - mark as initialized anyway
        useAuthStore.setState({ isInitialized: true });
      }
    } else {
      // Not authenticated - mark as initialized
      useAuthStore.setState({ isInitialized: true });
    }
  } catch (error) {
    // Silent fail - user might not be authenticated via web session
    console.debug('No session token available:', error);
    useAuthStore.setState({ isInitialized: true });
  }
};
