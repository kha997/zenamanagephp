...
| Project management | Tenant-scoped project CRUD, details, team linkage | `app/Http/Controllers/Api/ProjectController.php`; `app/Services/ProjectService.php`; `app/Models/Project.php`; `/api/zena/projects`; `/app/projects` | canonical | high | keep | Canonical business API is `/api/zena/projects`; `/api/v1/projects` remains mounted as `Src\CoreProject` compatibility runtime |
