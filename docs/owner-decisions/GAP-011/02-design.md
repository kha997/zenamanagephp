...
decision_provenance:
  ...
  owner_response_reference: "Owner Gate 2 round 1 decision — CHANGES REQUESTED, occurred in-session on 2026-08-13 (exact wall-clock time of the Owner's message not independently captured at the time; this frontmatter correction was written at the recorded_at timestamp above, retroactively, as part of the round-2-CHANGES-REQUESTED governance-history repair). Nine corrections requested: (1) test/redirect-existence-in-a-regression-test is not consumer evidence by itself — re-evaluate _debug/test-permissions, _debug/test-api-admin-stats, _debug/test-login-simple, _debug/test-session-auth against real-consumer evidence, default REMOVE absent a separately justified compatibility requirement; (2) _debug/test-login/{email} stays KEEP but the rationale must say it is a verified local/testing/development developer-login workflow, not a production consumer — resources/views/auth/login.blade.php's quick-login links only render inside app()->environment(['local','testing','development']); (3) testing-suite self-test hub: Owner decision is RETIRE, do not repair its links merely to justify keeping its children; (4) resolve the route-cache question inside Gate 2 itself, not defer to Gate 3 — the authoritative production workflow runs route:cache on the target host (git pull -> composer/npm -> migrate -> config:cache -> route:cache -> view:cache), so it does not distribute one shared route-cache artifact across environments, and Option C should be recorded as compatible with this; (5) tighten defense-in-depth wording — environment-gated registration and DebugGateMiddleware both depend on Laravel environment state and are not fully independent defenses against an incorrect APP_ENV; state precisely what each layer protects against and that a falsely-configured production environment defeats both; (6) re-evaluate all 7 Class B redirects on real-consumer evidence, default REMOVE absent a verified consumer, keep /debug/{path?} only as an explicit local developer convenience alias with new regression coverage; (7) make the invariant executable via a static architecture guard scanning route source files for declaration ownership (not route:list action metadata, not proven reliable for this purpose) plus a runtime middleware-presence guard plus route-cache-surviving tests; (8) preserve Class C scope lock, do not modify dashboard-content.blade.php's adjacent _debug/dashboard-data defect, record it only as a separate follow-up candidate; (9) re-present the revised matrix with zero UNKNOWN rows remaining. Do not implement; do not edit routes/tests/runtime code; do not mark PR ready; do not merge; do not self-approve Gate 2."
  reconciliation_required: false
supersedes: null
superseded_by: "docs/owner-decisions/GAP-011/02-design-v2.md"
timestamps:
...
## [FROZEN — ROUND 1, SUPERSEDED BY 02-design-v2.md] OWNER GATE 2: CHANGES REQUESTED — design surface for Class A + Class B only

**Governance note (added retroactively at `2026-08-13T16:58:30+07:00`, as part of repairing this work item's decision-immutability chain — see `02-design-v3.md` §0 for the full repair record):** this file's body below is preserved verbatim as originally authored and committed at `c2f8f4455d8ce27a8df88dad1a5fe6911799e6db` (`2026-08-13T16:38:44+07:00`). It represents round 1 of GAP-011 Gate 2 design, which the Owner reviewed and returned **CHANGES REQUESTED** on (nine corrections — see `decision_provenance.owner_response_reference` in this file's frontmatter for the verbatim list). At the time this file was originally committed, its frontmatter incorrectly left `gate_status: awaiting_owner` / `owner_decision.value: none` unchanged after that decision was received — this correction fixes the frontmatter only; **no body text below has been altered.** The corrected, superseding design is `02-design-v2.md` (round 2, itself superseded — also CHANGES REQUESTED) and `02-design-v3.md` (round 3, the active packet).

## OWNER GATE 2 (round 1, as originally presented): AWAITING OWNER DECISION — design surface for Class A + Class B only
