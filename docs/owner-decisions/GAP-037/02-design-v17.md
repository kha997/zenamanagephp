...
## 0. What changed vs. v16, at a glance

| # | Finding | Fix |
|---|---|---|
| 1 | §2.2 check #6's stated locking ("route lock, acquired before the `treasury_financial_documents` insert") put class 6 before class 5 — the opposite of the packet's own global 1→2→3→4→5→6 order | Corrected: reversal creation against a `via_route` original now acquires the original's `treasury_financial_documents` row (class 5) **before** reading its route (class 6). Every "6 → 5" statement removed; §11's worked-ordering example rewritten to "5 → 6" |
| 2 | Reversal *completion* never re-checked that a `via_route` original's route was still `completed` at the moment of completion — only reversal *creation* checked this (§2.2 check #6, v16). Between creation and completion, an intervening leg reversal (§4.2b) can regress the original's route from `completed` back to `partial`, and completion would proceed anyway, undoing an economic fact that, by then, had stopped being fully real | New §2.2e: at the reversal's own economic completion (§2.2a), if the original's `posting_path = via_route`, its route must **still** be `status = 'completed'` at that exact moment, re-checked under lock — not just at creation. If the original's route has regressed to `partial`, completion is rejected (not creation — the reversal document itself may still exist) until the original's route reaches full-target `completed` again |
| — | When both the original's route and the reversal's own route are locked in the same completion transaction (both documents `via_route`), §11 now states a deterministic intra-class-6 order: lower route `id` first, mirroring the existing lower-`id`-first rule already used for two `treasury_financial_documents` rows in class 5 |
| — | §18 gets 5 new adversarial scenarios (18.15c-g) proving: rejected completion after an intervening original-leg reversal; successful completion after the original route is restored by a replacement leg; a concurrent original-leg reversal racing reversal *creation*; a concurrent original-leg reversal racing reversal *completion*; and deterministic class-6 ordering when both original and reversal are `via_route` |

Everything else in this document is either byte-for-byte restated from v13-v16 (marked "held constant" where useful for traceability) or the fixes above.
...
### 2.1a Immutable posting and the closed status-transition graph (v16's addition, restated — restores Architecture C)
...
- `posted_unreconciled ⇄ posted_reconciled` — forward via §12.1's completeness condition, inverse via §12.2's regression — **bidirectional, but only while `status ≠ reversed`** (§12.2's exclusion, v16's fix, restated, below).
- `(posted_unreconciled | posted_reconciled) → reversed` — via §2.2a's completion moment. One-way.
- **`reversed` is terminal, unconditionally.** No write path in this schema ever transitions a document's `status` away from `reversed`, for any reason — not reconciliation regression (§12.2, v16's correction, restated), not any other event this schema defines. Once `status = 'reversed'`, it never changes again.
...
### 2.1b Positive-magnitude constraint (v16's addition, restated)
...
6. **Full-target completion for `via_route` originals (v16's addition, restated):** if `reversed_document.posting_path = 'via_route'`, its linked `treasury_payment_routes` row must additionally have `status = 'completed'` (§4.3's full-target predicate) at the moment of reversal creation. Check #5 alone is insufficient for `via_route` originals, since §5a's per-leg progressive posting lets `status` reach `posted_unreconciled` while the route is still only `planned`/`partial` — i.e. before the document's own economic movement has actually, fully happened. A reversal may never be created against a `via_route` original whose route has not yet reached full-target `completed`. **No equivalent extra check exists for `posting_path = direct` originals** — a `direct` posting is a single atomic action (§2.1a), so it is always either fully posted or not posted at all; check #5 alone is already sufficient for that branch. **Locking (corrected, this round):** this check reads the original's route under the route lock (§11 item 6), acquired **after** the `treasury_financial_documents` row (§11 item 5) — the original's row is already locked/read as part of checks #1-#5 (e.g. check #1's `UNIQUE(reversed_document_id)` requires reading the original), and the new reversal row is inserted into that same class 5, before this check reads the route under class 6. **v16 stated this backwards** (route lock acquired before the document lock) — a direct violation of the packet's own global 1→2→3→4→5→6 order; every "6 → 5" statement anywhere in this packet is corrected to "5 → 6" this round (see §11).
...
### 2.2d Advance full-reversal precondition — settlement net must be zero (v16's correction, restated)
...
### 2.2e Original-route re-verification at reversal completion (new, this round)

§2.2 check #6 verifies, **at reversal creation**, that a `via_route` original's route has reached full-target `completed`. That fact is not permanently frozen by the check — a route that was `completed` at creation time can later regress to `partial` if one of its legs is reversed (§4.2b), for example to correct a wrongly-settled leg pending a replacement. Nothing before this round re-verified the fact at the moment it actually matters: **reversal completion** (§2.2a), when the original's `status` is about to be permanently flipped to `'reversed'` and, per §2.2b, its dependent state is about to be atomically unwound.

**Binding rule:** if `reversed_document.posting_path = 'via_route'`, then at the exact moment of the reversal's own economic completion (§2.2a's completion moment, whichever of its two branches applies to the *reversal's own* `posting_path` — this check fires either way, since it is keyed to the **original's** `posting_path`, not the reversal's), the original's linked `treasury_payment_routes` row must **still** have `status = 'completed'` (§4.3's full-target predicate), re-checked under lock at that moment. If the original's route has regressed to `partial` (or, in principle, `cancelled`) since reversal creation, **completion is rejected** — not creation: the reversal document itself continues to exist exactly as it was (`draft`/`submitted`/`approved`, or posted-but-not-yet-completed for a `via_route` reversal whose own route has not reached `completed`), but the write that would flip the original's `status` and fire §2.2b's coupling is refused, atomically — the same style of completion-time re-check §2.2d already uses for an advance's settlement net.

**This is not a one-time rejection.** If the original's route is later restored to full-target `completed` (e.g. a replacement leg settles, bringing `net_custody` back to the required target per §4.3), a subsequent completion attempt for the same reversal document re-evaluates this check fresh and succeeds if every other applicable condition (§2.2a's own completion predicate, and, if the original is `document_type = 'advance'`, §2.2d's settlement-net-zero precondition) also holds at that later moment.

**Locking:** the original's route (§11 item 6) is read under lock as part of the same completion transaction that locks the `treasury_financial_documents` row(s) (§11 item 5, lower `id` first per §2.2a) — item 5 before item 6, per §11's corrected global order. **When both the original's route and the reversal's own route are locked in the same completion transaction** (the original is `via_route`, per this section, *and* the reversal itself is also `via_route`, triggering §2.2a's own `via_route` completion branch on its own route): the two route rows are locked in a deterministic order — **lower route `id` first** — mirroring the existing lower-`id`-first rule §2.2a already uses for two `treasury_financial_documents` rows. See §11 for the complete, corrected lock-order statement and §18.15c-g for worked scenarios.
...
### 7.4a Two non-interchangeable reversal mechanisms for `treasury_advance_settlements` (v16's addition, restated — disambiguates §2.2d)
...
**There is exactly one global acquisition order: 1 → 2 → 3 → 4 → 5 → 6.** Every transaction that needs more than one of the six classes below acquires them in strictly ascending class order — never the reverse, never interleaved by which entity is conceptually "primary" to the write. Two contradictions of this rule have been found and corrected across prior rounds: v15 stated the document row (item 5) locking before the advance row (item 3) for advance posting — corrected in v16 (item 3 always before item 5). v16 then stated the route lock (item 6) acquired before the document lock (item 5) for reversal-creation's `via_route`-original check — **corrected this round: item 5 always locks before item 6, with no exception, in every write in this schema, including this one.**
...
| 5 | Financial-document posting-source selection, Case B equality + immutability freeze, structural eligibility gate, §2.2's creation-time reversal checks including the no-reversal-of-reversal rule and the `via_route`-original full-target-completion precondition (check #6), §2.2a's branched atomic status-flip, §2.2b's dependent-state coupling, §2.2d's advance-reversal precondition, §2.2e's original-route re-verification, direction/linkage rules, expense approval gate | `treasury_financial_documents` row | When two document rows are locked together (an original + its reversal): lower `id` first, independent of which is "original" vs. "reversal" |
| 6 | Route-leg custody, full-target endpoint-aware completion, atomic leg-reversal/ledger-reversal coupling, the `via_route` reversal-completion trigger (§2.2a), the `via_route` `advance_return`-settlement-completion trigger (§7.6), the `via_route`-original full-target-completion read for reversal creation (§2.2 check #6), **the original-route re-verification read at reversal completion (§2.2e, new this round)** | Parent `treasury_payment_routes` row | **Corrected this round:** a reversal's own route and the original's route are **not** always distinct-and-never-both-locked — when the original is `via_route` (§2.2e fires) *and* the reversal itself is also `via_route` (§2.2a's own completion branch fires on its own route), **both** route rows are locked in the same completion transaction: **lower route `id` first**. When only one of the two applies, the single row locked is whichever one is relevant; no ordering ambiguity arises with only one row |

**Worked orderings, restated to make the corrected order unambiguous:**
- **Advance posting (§7.5):** 3 → 5.
- **Advance-return posting, `direct` (§7.6):** 3 → 5.
- **Advance-return posting, `via_route` completion (§7.6):** 3 → 5 → 6.
- **Reversal creation against a `via_route` original (§2.2 check #6, corrected this round):** 5 (lock/insert the reversal row against the already-read original document row) → 6 (read the original's route completion state).
...
- **Reversal completion, `via_route`, original `document_type = advance_return`:** 3 (§2.2b's `cash_return`/`reverse` coupling touches the advance) → 5 (lower id first) → 6 (§2.2e's re-verification of the original's route).
- **Reversal completion, `direct`, `via_route` original that is not `advance`/`advance_return`/`expense` (e.g. `internal_transfer` or `funding`):** 5 → 6 (§2.2e's re-verification only — no item 2/3 coupling applies to these types).
- **Reversal completion where *both* the original and the reversal are `via_route` (new worked example, this round):** 5 (lower id first, original+reversal) → 6 (both route rows, **lower route `id` first** — §2.2e's original-route re-verification and §2.2a's own reversal-route completion trigger both fire in the same transaction, against two distinct route rows).
...
1. `treasury_financial_parties` 2. `treasury_wallets` 3. `treasury_financial_documents` 4. `treasury_payment_routes` 5. `treasury_payment_route_legs` 6. `treasury_ledger_entries` 7. `treasury_fund_chains` 8. `treasury_advances` 9. `treasury_advance_settlements` 10. `treasury_cost_settlement_allocations` 11. `treasury_expense_approvals` 12. `treasury_reconciliations` 13. `treasury_fund_chain_members` 14. `treasury_reconciliation_entries`. **No migration file exists yet.** This round adds **zero new tables, zero new columns, zero new indexes** — every correction (§0) is purely a lock-ordering and completion-time-recheck rule over columns, indexes, and locks that already existed as of v16. (v16 itself added 7 new `CHECK (> 0)` constraints, §2.1b, on existing amount columns — restated, unchanged, this round.)

---

## 17. Verification against every held-constant item

- A3/A4-a/A.5/B2/B2-T/C/D: unchanged, not reopened. A4-a exclusion held. D — no `ReportPageController::cashflow()` edit: none of this round's fixes touch that controller. **C — immutable posting**: restored in v16, unaffected by this round's lock-order/concurrency corrections.
- B2-T no-second-`ContractPayment`-fact: unaffected — this round adds no new read or write against `ContractPayment`.
- 14-table count, 12 composite-FK targets, source/destination party provenance, exact endpoint reversal shape, explicit Case A endpoint, full-target completion predicate, posting matrix, leg-reversal atomicity, many-to-many cost allocation, approval/reconciliation lifecycle, Tier-B/project isolation, zero existing-table/data migration, the §2.1a immutable-posting/closed-transition-graph rule, the §7.4a manual-vs-coupled reversal distinction, the §2.2d settlement-net formula, the §12.2 reversed-exclusion, positive-magnitude constraints: all restated, all unchanged in substance from Revision 16.
- **This round's correction (lock order, §2.2 check #6 and §11):** verified as a pure ordering fix, not a new check or a removed one — check #6's *content* (the original's route must be `completed` at reversal-creation time) is byte-for-byte unchanged from v16; only the *sequence* in which its two locks are acquired is corrected, from 6→5 to 5→6, matching the packet's own stated global order for the first time. No reversal that was validly creatable under v16's (wrongly-ordered but logically-equivalent, in the single-threaded case) check is now invalid, and no invalid one becomes valid — the correction only matters under genuine concurrency, which is exactly why it needed the dedicated adversarial scenarios below rather than a single-threaded re-check.
- **This round's correction (§2.2e, completion-time re-verification):** verified as strictly additive — a new completion-time rejection case for `via_route` originals whose route has regressed since reversal creation. No reversal that was previously able to complete (because its original's route was, and remained, `completed` throughout) is newly blocked; only the specific case where the original's route regressed *after* creation and *before* completion — previously unguarded — is now caught.
- **This round's correction (§11, dual-route deterministic ordering):** verified as additive and necessary, not previously reachable as a code path that could deadlock under v16 (since v16 never let a completion transaction lock two distinct route rows at once — §2.2e is what first makes that dual-lock case real) — this round closes the gap at the same time it opens the code path, so no window of under-specified concurrency ever existed for this specific combination.
...
**18.15 Reconciliation and reconciliation reversal, not yet document-reversed.** A posted `direct` `funding` document's single `credit` entry gets a `treasury_reconciliation_entries` row (`direction=apply`) under a `treasury_reconciliations` batch for its wallet. §12.1: document → `posted_reconciled`. A `direction=reverse` reconciliation-entry is later created (`reverses_reconciliation_entry_id`→ the apply row) — §12.2: since the document's status is `posted_reconciled` (not `reversed`), the regression fires normally: document → `posted_unreconciled`, atomically. This is fully orthogonal to document-level reversal (§2.2 is a different mechanism from reconciliation, §12, per §5's four-mechanism disambiguation) — no interaction, no shared lock beyond item 4 (`treasury_ledger_entries`).

**18.15a Reconciliation reversal on an already economically-reversed document (v16's scenario, restated — proves the §12.2 exclusion).** Take 18.4's `internal_transfer` X, already reconciled (`posted_reconciled`) *before* it was later reversed (18.4: X → `reversed` via its own `reversal` document completing). X's original `credit`/`debit` entries still each carry an active `apply` reconciliation-entry row from before the reversal. Someone now creates a `direction=reverse` reconciliation-entry against one of those old entries (`reverses_reconciliation_entry_id`→ the apply row) — perhaps a delayed bank-statement correction. §12.2 (v16's correction, restated): the `treasury_reconciliation_entries` row is still created normally (the reconciliation bookkeeping fact is recorded), **but** since `X.status = 'reversed'` at that moment, the regression side-effect is suppressed — X's `status` does **not** move back to `posted_unreconciled`. Final state: the reconciliation-entry table gained a row (an honest record of the correction event), but `X.status` remains permanently `reversed`, consistent with §2.1a's closed transition graph — the terminal fact §2.2a established is never disturbed by a later event in an unrelated subsystem (reconciliation).

**18.15b Reversal creation rejected against a partial `via_route` original (v16's scenario, restated — proves §2.2 check #6).** `funding` document F=100, `via_route`-eligible (`destination_wallet_id` set), attaches a Case B route, `total_allocated_amount=100`. Leg 1 settles 40; `F.status` has already progressed to `posted_unreconciled` per §5a's progressive per-leg posting, while the route itself is still `status='partial'` (`net_custody(destination)=40≠100`). Someone attempts to create a `reversal` document targeting F (`reversed_document_id=F.id`). §2.2 check #5 alone would pass (`F.status IN (posted_unreconciled, posted_reconciled)` — it is `posted_unreconciled`). **§2.2 check #6 (v16's fix) additionally evaluates:** `F.posting_path='via_route'` → its route must have `status='completed'`; it is `'partial'` → **creation is rejected**. No reversal row is ever persisted against F until its route reaches full-target `completed` (leg 2 settles the remaining 60). Final state: no reversal ever exists against an economically-incomplete `via_route` original; once the route completes, the identical reversal-creation request succeeds normally under checks #1-#7.

**18.15c Completed original → create reversal → original leg reversed → reversal completion rejected (new, this round — proves §2.2e).** `funding` document O=100, `via_route`, Case B route Ro (`total_allocated_amount=100`) reaches full-target `completed` (`net_custody(destination)=100`). A `reversal` document targeting O is created — §2.2 check #6 passes (Ro is `completed` at creation time). Before the reversal completes, one of Ro's legs (say the one carrying 60 of the 100) is reversed via §4.2b's atomic leg-reversal coupling — this is an ordinary leg-level correction (e.g. discovered miscount), independent of the pending reversal. Ro's `net_custody(destination)` drops to `40 ≠ 100`, so Ro regresses to `status='partial'` (§4.3's existing rule — a route with a reversed leg stays `partial` until a full-target replacement completes it; this round makes explicit that this regression is visible to, and binding on, §2.2e's completion-time check). The reversal now attempts to complete (`direct` branch, for concreteness). §2.2a's own completion write proceeds to §2.2e's re-verification: reads Ro under the item-6 lock (after item-5, per the corrected order) — `Ro.status='partial' ≠ 'completed'` → **completion is rejected**. Final state: O's `status` remains whatever it was before (`posted_unreconciled` or `posted_reconciled` — never `reversed`), the reversal document persists unchanged (still not completed), and no dependent-state coupling (§2.2b) ever fires — the schema never records O as undone while its own underlying route no longer proves the full amount actually reached its target.

**18.15d Original route restored by replacement leg → reversal completion succeeds (new, this round — proves §2.2e's non-permanence).** Continuing 18.15c: a replacement leg for the reversed 60 settles, bringing `net_custody(destination)` back to `100`; §4.3 re-evaluates Ro to `status='completed'`. The same reversal document (still pending, never having completed) is submitted for completion again. §2.2e re-checks fresh: `Ro.status='completed'` → holds. §2.2a's `direct`-branch completion proceeds: O.status→`reversed`, and (since O has no dependent-state coupling of its own type, `funding`) no further coupled write. Final state: identical to what 18.15c would have produced had the intervening leg-reversal never happened — the schema's terminal outcome is path-independent of *when* the original's route reaches its final, correct `completed` state, only *that* it does, at the moment reversal completion actually commits.

**18.15e Concurrent original-leg reversal vs. reversal creation (new, this round — proves the corrected 5→6 creation order under contention).** O=100, `via_route`, route Ro currently `completed`. Two transactions start concurrently: (T1) create a `reversal` document targeting O (§2.2, including check #6); (T2) reverse one of Ro's legs (§4.2b). T1 acquires item 5 first (locks/reads O's document row, inserts the new reversal row) then attempts item 6 (reads Ro). T2 needs only item 6 (the parent route lock, §4.2b). The two transactions serialize on item 6: whichever reaches it first wins. **If T2 commits first:** Ro is already `partial` by the time T1's check #6 reads it → **reversal creation is rejected** (no reversal row persists, or T1's row-plus-check fails atomically and rolls back). **If T1's item-6 read happens first** (holding the lock until T1 commits): T1 sees `Ro='completed'`, passes, and commits the new reversal row; T2 then proceeds, reverses the leg, and Ro regresses to `partial` — strictly *after* T1's creation-time check already correctly recorded `completed` as of that moment. Either interleaving is internally consistent; no interleaving lets T1 read a value of `Ro.status` that was already stale when it made its decision, because both transactions serialize through the same single row lock.

**18.15f Concurrent original-leg reversal vs. reversal completion (new, this round — proves §2.2e under contention).** Continuing from a state like 18.15e where the reversal was successfully created against `completed` Ro: two transactions now race — (T1) the reversal's own completion write (item 5, then item 6 for §2.2e's re-check); (T2) a leg reversal on Ro (item 6). Serialized on item 6 exactly as in 18.15e. **If T2 commits first:** Ro is `partial` when T1's §2.2e check reads it → **completion rejected** (18.15c's scenario, reached via concurrency instead of sequential timing). **If T1's item-6 read happens first:** §2.2e passes, T1 completes atomically (O.status→`reversed`, any applicable §2.2b coupling fires, all under the still-held item-6 lock alongside item 5), then T2 proceeds and reverses the leg — Ro regresses to `partial`, but O is already permanently `reversed` (§2.1a's terminal-state rule) by then; the leg-level correction is still recorded normally at the route/ledger layer (no rule in this schema forbids a leg-level correction on the route of an already-`reversed` document, the same way §12.2's exclusion still lets reconciliation-entry rows record normally on a `reversed` document — only the *document's own status* is protected as terminal, never the lower-layer bookkeeping about it), but it can no longer retroactively invalidate a reversal that has already, correctly, completed.

**18.15g Both original and reversal are `via_route` — deterministic class-6 ordering (new, this round — proves the dual-route-lock rule).** O=100, `via_route`, route Ro (`id=501`) `completed`. A `reversal` targeting O is created and itself also resolves to `posting_path='via_route'`, attaching its own route Rr (`id=502`) — a distinct row from Ro (routes are 1:1 per financial document via Case B's `UNIQUE(linked_financial_document_id)`, and Rr's `linked_financial_document_id` is the *reversal* document's id, not O's). When Rr's legs reach full-target `completed`, the reversal's own completion transaction fires: §2.2a's `via_route` branch (triggered by Rr reaching `completed`) **and** §2.2e's re-verification (triggered because O's `posting_path='via_route'`) both need a route-row lock in the *same* transaction — Ro (for §2.2e's read) and Rr (for §2.2a's own trigger, already implicitly locked as the route that just reached `completed`). Per §11's corrected item-6 rule: locked **lower route `id` first** — here, `Ro(501)` before `Rr(502)`, regardless of which of the two routes' legs happened to settle to `completed` last in wall-clock time. Final state: O.status→`reversed` (both §2.2a's own trigger and §2.2e's re-check passed), §2.2b's coupling fires if applicable to O's `document_type`, and the fixed `Ro-before-Rr` ordering means a *second*, hypothetical concurrent transaction that needed both of these same two route rows (however unlikely — e.g. some future extension touching both) could never deadlock against this one, since both would always acquire them in the identical `501-before-502` order.
...
**Audit conclusion:** no scenario above surfaces a further inconsistency inside the approved architecture (A3+A4-a+A.5/B2+B2-T/C/D). The fixes in §0, as specified, close every point in the Owner's Revision 16 REQUEST CHANGES decision and hold under the full scenario set — including, this round, the five dedicated adversarial scenarios (18.15c-g) targeting exactly the via_route-original ↔ document-reversal concurrency boundary the Owner's finding identified, plus every scenario from prior rounds re-verified still holding against the corrected lock order and the new §2.2e re-check.
