...
**B. Cash authority** — 2 options considered: (B1) Treasury absorbs `ContractPayment` too (high migration risk, contradicts SSOT §7.2's "not a duplicate of the Contract payment lifecycle"); (B2, **recommended**) Treasury becomes canonical only for cash movements `ContractPayment` doesn't already cover — supplier/labor/material payments, owner contribution, internal transfers, advances/settlements. `ContractPayment` keeps owning client-payment cash-in, untouched. **§B.1, added this round:** Treasury may record custody/route/wallet facts about a `ContractPayment`-sourced event (the A→C→Y multi-leg case) — but only ever linked back to that `ContractPayment`, with `sum(route legs) <= ContractPayment.amount` enforced, so the commercial "paid" fact is asserted exactly once, and Treasury's route/wallet facts are a location projection of it, never a second funding event.
...
- **Round 2 (this revision):**
  1. Spec §B.1 (new): resolved ContractPayment↔Treasury funding traceability for the A→C→Y multi-leg case — binding relationship, not deferred.
...
**Round 2 (đã xử lý):** Owner Request changes, tại PR #263 head `6f7dd74402ce29acd96ce6b117054e064c192e58` (2026-08-16) — xác nhận 4/5 điểm Round 1 đạt yêu cầu; 2 correction bắt buộc còn lại: (1) sửa funding-route conservation invariant (không cộng leg amount, dùng allocation-level conservation); (2) đổi tên §B.1 thành §B2-T để không xung đột với Option B1 (không được chọn). Chi tiết nguyên văn lưu trong lịch sử git tại commit ghi Round 2 decision. Xem `## Revision log` phía trên cho tình trạng xử lý.
