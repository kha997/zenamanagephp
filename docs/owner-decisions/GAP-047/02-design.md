...
- **Round 2 / FINAL (PR #290 head `6294a159960ec13e01f79346624d090cbdf1f2c1`,
  canonical main independently reverified
  `6a371405feeb44b644dcf16e76ee1c1a214c7134`):** Owner **APPROVES**, with two
  binding implementation clarifications. The A1/B3 architecture and all three
  Round 1 corrections (non-self-referential `snapshot_baseline_sha`, zero
  filename pass/fail authority, no permanent generator script) are accepted
  as revised; regression cases 15–19 are accepted. `gate_status` is now
  `approved`, `owner_decision.value` is `approved`. This approval
  **authorizes implementation planning and implementation only within the
  design and the two binding clarifications below** — it does not itself
  start implementation or open Gate 3.
  - **Binding Clarification A — grandfather set may not silently expand
    after Owner review.** The 103-file historical set reviewed at Gate 2 is
    the *maximum eligible* grandfather universe. At implementation start the
    plan must: fetch latest origin, determine current canonical
    `origin/main`, record it as `snapshot_baseline_sha`, recompute the
    current no-complete-frontmatter path set against that tree, and diff it
    against the Owner-reviewed historical set. A path that was in the
    reviewed set may still be grandfathered by exact path even if still
    missing frontmatter; a reviewed path that gained frontmatter or
    disappeared needs no entry — so the implementation-time set may stay
    equal or *shrink*, but must never automatically *grow*. Any new
    missing-frontmatter spec/plan path on latest main that was **not** in
    the Owner-reviewed historical inventory must **STOP** implementation and
    be reported as drift — it may not be silently grandfathered or
    reinterpreted as "historical" merely for predating the implementation
    branch cut; it must either receive proper frontmatter through an
    authorized path or a separate explicit Owner exemption decision. The
    implementation plan must make this set-difference check explicit.
  - **Binding Clarification B — grandfather configuration load must fail
    closed.** The implementation must define and test deterministic loading
    semantics for
    `docs/owner-governance/grandfathered-nonfrontmatter-documents.txt`:
    missing/unreadable file, malformed entry, absolute path, and
    path-traversal (`../`) entries are all hard lint failures (never a
    silently-empty grandfather set); non-comment/non-blank entries must be
    exact repo-relative `.md` paths under exactly
    `docs/superpowers/specs/` or `docs/superpowers/plans/`; duplicate
    entries must be rejected, not silently normalized; blank lines and `#`
    comments may be ignored; filename casing or apparent Work ID never
    changes exemption semantics. Focused regression tests are required for
    every one of these fail-closed configuration cases.
  - **Test/artifact clarification.** "Single script plus one data file"
    describes the behavioral correction surface, not the complete PR
    changed-file count — the implementation PR is expected to add/change the
    regression test files required by the approved 19-case matrix plus the
    implementation plan; tests must not be omitted to preserve a literal
    two-file diff. No workflow-file change is authorized unless
    implementation proves the script-level design cannot satisfy the
    contract — if that becomes necessary, **STOP** and return to Owner
    review before adding it.
  - This approval-record update (`docs/owner-decisions/GAP-047/02-design.md`
    only) is itself the Gate-2 approval action; the engineering design and
    spec are unchanged from Round 1's revision — no mechanical reference
    update was needed. Implementation is explicitly **not** started in this
    session; Gate 3 is explicitly **not** opened; GAP-046 / PR #288 is not
    touched. Full verbatim directive preserved in
    `decision_provenance.owner_response_reference` above (appended after the
    Round 1 verbatim text, separated by ` | `).
