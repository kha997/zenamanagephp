...
**Gate 3 packet status: `awaiting_owner`.** Technical readiness: `ready`.
This packet requests an Owner Gate-3 decision. It does not record one.

## SHA identity — read this before anything else

Three distinct SHAs appear in this packet and its history. They are not
interchangeable:

1. **Implementation subject SHA** — `422c5015677867734e64ab2149069d426492b9ff`.
   This is the exact commit whose tree the `implementation_tree_digest`
   below was computed against. It is the head of PR #308 at the moment
   this packet was authored, and it is the last commit that touched any
   implementation or evidence-document file (i.e. anything other than
   this exact Gate-3 packet).
2. **Implementation-tree digest** —
   `203856e77d741ec45c86fb7eac16fcf72f4c7d247be6e3f3f209cd519a26ddc1`.
   Computed via `owner_governance_compute_implementation_tree_digest()`
   (`scripts/ssot/owner_governance_lint.php`) at the subject SHA above: a
   SHA-256 of the sorted `"<blob_sha> <path>"` lines for the complete git
   tree, excluding every `docs/owner-decisions/*/03-release*.md` file in
   the repository (this repo's Gate-3 self-reference exclusion, not
   limited to `GAP-051/`'s own directory). Per the packet-schema's own
   design, a commit that touches ONLY this exact file does not change
   this digest, by construction.
3. **Packet-only PR head SHA** — not yet known at the time this file is
   authored, since this packet has not yet been committed/pushed. Once
   this file is committed and pushed as an isolated, packet-only commit,
   PR #308's head will advance past `422c5015...` to that new commit. Per
   point 2, that later head will **not** represent a new implementation
   subject — the implementation tree remains byte-identical to
   `422c5015...` — and the coordinating report for that push will record
   the new head explicitly as a packet-only commit, never silently
   conflated with the implementation subject SHA above.
