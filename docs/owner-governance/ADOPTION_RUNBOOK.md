...
2. Implementation, tests, review, CI proceed per the normal `superpowers:writing-plans` → `superpowers:subagent-driven-development` flow — nothing about this changes. **This includes committing packet-only updates to the Gate 3 record itself as work progresses** — per Correction 2, a commit that touches only `docs/owner-decisions/<work-id>/03-release.md` never changes `implementation_tree_digest`, so refreshing the packet's narrative mid-flight is always safe and never self-invalidates.
