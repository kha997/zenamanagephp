...
- **`external_boq_project_code`/`external_quote_snapshot` are excluded from `Api\OpportunityController::update()`'s general-purpose validation/fillable list.** They are only ever set through the two dedicated, gate-checked endpoints built in this plan — mirroring the precedent set for `review_status` in the sibling Design Item feature (never mutable through a generic `update()`).
