...
Found during the user's live walkthrough (2026-07-20) alongside a related, explicitly **out-of-scope** finding: the app has no professional role taxonomy (Architect/Engineer/Site Supervisor etc.). `RoleSeeder` only creates three permission-bearing roles (System Admin/Project Manager/Project Member) via the `roles()` pivot relationship, and those specific role names are never displayed in the UI. Two other surfaces *do* display a role label — `resources/views/app/profile.blade.php:16` (`auth()->user()->role`) and `resources/views/app/team-content.blade.php:48` (`member.role`) — but both read a separate, free-text `role` string column on `User`, unrelated to `RoleSeeder`'s pivot-based roles; neither surface is affected by or relevant to this page. This page filters by `assigned_to = auth()->id()`, which is orthogonal to any role concept, pivot-based or the free-text column. Role taxonomy is deferred to its own future brainstorm.

## Scope

**In scope:**
- New route `GET /app/my-work` (route name `app.my-work.index`), gated by the existing `rbac:task.view` — the same middleware already on `app.workload.index` (`routes/web.php:387`). Note: `app.tasks` (`routes/web.php:386`) carries **no** middleware at all; this page's gate matches `workload.index` specifically, not `tasks`. No new permission either way.
...
New `tests/Feature/MyWorkPageTest.php` (fixture pattern copied from `WorkloadPageTest`): (1) authorized user sees only their own open task and design item; (2) **explicit non-render assertions**: a same-tenant coworker's open task/design item does NOT appear on `/app/my-work` (assert the coworker's item name is absent from the response, not merely that the viewer's own items are present — a page that accidentally rendered everyone's items would still pass a presence-only check), and an unassigned open item (no `assigned_to`) also does NOT appear (this page has no "Chưa phân công" concept, unlike #195's manager view); (3) the count line matches (N/M/K) for a fixture with one overdue task and one blocked item; (4) user without `task.view` → 403; (5) cross-tenant isolation — another tenant's user with the same items never appears; (6) a user with zero assigned open items sees the empty-state message, not an error. `WorkloadPageTest` itself is unchanged (the extraction must not alter `index()`'s existing behavior — its current tests remain the regression guard).
