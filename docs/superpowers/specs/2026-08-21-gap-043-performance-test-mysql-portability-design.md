...
**Precise characterization of the portability contract (corrected from v1):** `Schema::getColumns()` is portable at the *Laravel API boundary* — it gives both drivers the same structured keys (`name`, `nullable`, `default`, `type_name`, `generation`, …) via per-driver query processors (`MySqlProcessor::processColumns()`, `SQLiteProcessor::processColumns()`). This does **not** mean SQLite and MySQL return byte-identical raw `default` representations — they don't (see LOCAL probe evidence immediately below, which shows exactly how they differ). The contract Option A actually relies on is weaker and more precise: *common structured keys, semantically equivalent default information, and a normalization step (already present in `tableInsertDefaults()`) sufficient for this helper's actual load-bearing defaults* — all 12 of which are plain literal values (strings, integers, decimals), not SQL expressions. The `generation` key is evidence about *computed/generated columns* specifically (`STORED GENERATED`/`VIRTUAL GENERATED`); it is not evidence that `Schema::getColumns()` handles arbitrary `DEFAULT` expressions (e.g. `DEFAULT (CURRENT_TIMESTAMP)`) — that question is separate and, for GAP-043, moot: **none of the 12 load-bearing defaults are SQL expressions**, confirmed directly by the LOCAL probe below (`generation` is `null` for every column in both `projects` and `tasks`, on both drivers, and all 12 `default` values are plain quoted/unquoted literals like `'low'`, `'0.00'`, `'1'`).
...
**`generation` field:** `null` for every column on both tables, both drivers — confirming none of the 26 probed defaults (and specifically none of the 12 load-bearing ones) are computed/generated columns or SQL default expressions; all are plain literal scalars, which is exactly the class of default `tableInsertDefaults()`'s normalization logic is designed to handle.
