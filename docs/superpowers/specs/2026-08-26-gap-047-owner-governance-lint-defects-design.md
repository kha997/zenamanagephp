...
**Revision note (Round 1):** PR #290's original submission (head
`eb4a6fa49466dabe69a37ba08e389c97b3e2ab30`) proposed B3 with a
self-referential snapshot-SHA header, a filename-regex path that could
still grant a PASS, and a generator-artifact scope contradiction. The
Owner accepted A1 and B3 in principle but required three corrections
before implementation authorization — see
`docs/owner-decisions/GAP-047/02-design.md` § Revision log for the full
verbatim directive. §3.3, §3.4, §5, and §5.1 below are revised accordingly;
§1, §2, §3.1, §3.2, §4 (except as noted), §6, §7, and §8's structure are
...
### 3.3 Recommendation: B3, with a two-part mechanism (revised per Owner Round 1)

**Part 1 — new grandfather-path file**,
`docs/owner-governance/grandfathered-nonfrontmatter-documents.txt`.

**Snapshot boundary — non-self-referential contract (Correction 1).** A
committed file cannot deterministically contain the SHA of the commit
whose SHA is itself determined by that file's own contents — the Round 1
draft's "Snapshot commit: <the exact commit SHA this correction is
implemented at>" header was invalid for exactly this reason. The corrected
contract:

1. At implementation start: `git fetch origin`.
2. Verify canonical `origin/main` (`git rev-parse origin/main`); record
   that exact SHA as **`snapshot_baseline_sha`**. If `main` has moved past
   `6a371405feeb44b644dcf16e76ee1c1a214c7134` by implementation time, use
   the newly verified SHA — this Gate 2's baseline is not hard-coded as a
   future implementation baseline.
3. Cut the implementation branch from that exact `snapshot_baseline_sha`.
4. **Before making any implementation mutations**, compute the historical
   non-frontmatter path set from the `snapshot_baseline_sha` tree — i.e.
   `git show <snapshot_baseline_sha>:docs/superpowers/{plans,specs}/*.md`
   (or an equivalent read against that exact tree, not the working tree
   post-mutation), filtered to files lacking complete frontmatter.
5. Write that set to the grandfather file, with `snapshot_baseline_sha` —
   a value fully determined *before* the implementation commit exists — as
   the file's header, not the implementation commit's own (not-yet-known)
   SHA:

```
# Snapshot baseline main:
# <exact verified canonical origin/main SHA the implementation branch was
#  cut from — recorded BEFORE any implementation mutation, per
#  docs/owner-decisions/GAP-047/02-design.md Correction 1. This is NOT
#  and must never be the SHA of the commit that adds this file.>
...
# and must carry frontmatter or fail. This file is a FROZEN HISTORICAL
# SNAPSHOT, not a rolling allowlist: removing an entry (because the file
# gained real frontmatter, or was deleted) is always safe; adding a NEW
# entry for a file that did not exist at the snapshot baseline requires
# its own reviewable, CODEOWNERS-gated governed change (same discipline as
# legacy-work-ids.txt) and must never become a routine way to skip writing
# frontmatter for new work — see case 19 in the regression matrix.
docs/superpowers/plans/2026-07-09-design-item-phase1.md
... (every non-frontmatter path at snapshot_baseline_sha, one per line,
    generated not hand-typed)
```

**The final implementation-head completeness proof replaces any need for
self-referential commit binding** (Owner's stated substitute mechanism):
regression case 18 (§5.1) independently re-derives, at the implementation
head, the set of `docs/superpowers/{plans,specs}/*.md` files lacking
complete frontmatter and asserts every one of them is on the grandfather
list — this is checked against the actual final tree, not against a claim
embedded in the file itself, so nothing in the design depends on the file
correctly predicting its own future commit identity.

**No permanent generator script (Correction 3).** The Round 1 draft
proposed both a committed `scripts/ssot/generate_grandfathered_nonfrontmatter_documents.php`
and, contradictorily, described the change as "one existing lint script
plus one new data file." Resolved in favor of the minimal option: **no
permanent generator script is added.** The grandfather file is produced by
a documented, deterministic **one-off command**, run once against the
verified `snapshot_baseline_sha` tree and recorded in the implementation
plan (e.g. a short one-off PHP or shell script executed from the CLI,
never committed under `scripts/ssot/`, or a `php -r` one-liner — the exact
form is an implementation-plan detail, not a Gate 2 commitment). This
keeps the change genuinely "one script file plus one new generated data
file," matching §7/§8's rollback description, and avoids adding a
standing tool whose only job is a single historical snapshot that must
never be silently re-run to "top up" the list later (§8, and case 19).
Regression tests (case 18) prove the resulting list is complete and — by
construction of the one-off command reading directly from the verified
tree — that no path in it was hand-invented.

Lives under `docs/owner-governance/` (governance configuration,
deliberately **outside** the design-only exemption prefixes — any future
edit to this file is not itself a design-only change and must go through
its own governed work item, exactly like `legacy-work-ids.txt` today).

**Part 2 — rewrite the frontmatter-detection branch** (replacing lines
515–570) with a strict three-way decision tree that gives filename
parsing **zero pass/fail authority** (Correction 2):

1. Parse frontmatter as today (lines 518–524).
2. **(A) Complete frontmatter exists** — `work_id` +
   `owner_governance_version` + `owner_gate_2_record` all present →
   proceed to today's existing Gate-2-record resolution logic (lines 572
   onward) **unchanged**.
3. **(B) Frontmatter exists but is incomplete** — `work_id` set, but
   ...
   **unchanged**.
4. **(C) No usable governance frontmatter at all** → check the file's
   exact relative path against the grandfather list, and *only* that:
   - Exact path **is** on `grandfathered-nonfrontmatter-documents.txt` →
     `continue` (exempt), byte-preserved, no violation. This is the
     **only** way a no-frontmatter file passes.
   - Exact path **is not** on the list → unconditional
     `missing-governance-frontmatter` violation, for **every** case: bulk
     scan or explicit target, filename carries a recognizable uppercase
     token, a lowercase token, a no-hyphen token, or no token at all. The
     `$scanningExplicitFiles` branch distinction (lines 548, 556) is
     removed for this rule — it no longer matters *how* the file was
     reached, only whether it is grandfathered.

That is the complete decision tree — bulk scan vs. explicit scan, filename
casing, and presence/absence of a recognizable Work-ID token change
**none** of it. `legacy-work-ids.txt` is **not** consulted anywhere in
this branch and grants **no** frontmatter exemption; it continues to serve
only its existing established purposes elsewhere in
`owner_governance_enforce_gate_ordering()` (the `in_array($workId,
$legacyIds, true)` check at line ~572, which only ever runs once a
frontmatter `work_id` is already known — i.e. case (A)/(B) above, never
case (C)). GAP-032's spec needs no filename-derived legacy exemption
because its exact path is already part of the grandfather snapshot (§3.1
Class 2) — it passes exclusively through case (C)'s path check, exactly
like the other 8 Class 3 files, with no special-casing.

**If a filename-token regex is retained at all**, its only permitted
effect is improving the violation message's wording when a
non-grandfathered file happens to have a recognizable token in its
filename (e.g. "File appears to reference GAP-047 by filename but is not
on the frontmatter-grandfather list and has no governance frontmatter" vs.
a generic message when no token is found) — this text differs, but the
violation rule name (`missing-governance-frontmatter`) and the fact that
it fails do not. Regression case 17 asserts this equivalence directly.

**Invariant restated exactly as required:** filename parsing has zero
pass/fail authority for missing-frontmatter enforcement.

### 3.4 Why this doesn't retroactively break `main`

The grandfather-path file's snapshot generation is exhaustive by
construction: the one-off generation command (§3.3 Part 1) reads the exact
same two directories the lint itself scans
(`docs/superpowers/plans/*.md`, `docs/superpowers/specs/*.md`) at the
verified `snapshot_baseline_sha` tree — not the working tree after
implementation edits — filters to files that do **not** already have
complete frontmatter, and writes every one of those paths out. The
104/103-style headline count in §3.1 is descriptive of the Gate 2 baseline
only; per Owner Round 1, Gate 2 does not bind implementation correctness
to that specific number — the exact generated set at implementation time
is whatever the one-off command actually finds against
`snapshot_baseline_sha`, which may differ if other PRs land first. Because
every currently-non-frontmatter file becomes either (a) a grandfather-list
entry or (b) forced to gain frontmatter as part of the same implementation
PR, a fresh bulk scan run immediately after the correction lands produces
**zero** new violations against the same tree that produced zero
violations before the correction. `main`'s `push`-triggered lint (line 85,
no changed-files scoping at all — a full bulk scan every time) is
therefore never made red by this change, given a complete snapshot. §5.1
below specifies the two regression tests that make this a tested fact
rather than an assertion: snapshot completeness (case 18) proves the
grandfather list is exhaustive at the implementation head; grandfather
immutability (case 19) proves the list cannot silently grow to absorb a
*new* no-frontmatter file introduced after the snapshot, which would
otherwise let a future PR quietly turn the "frozen historical snapshot"
into a rolling allowlist for its own omissions.
...
- Filename parsing has **zero** pass/fail authority under the revised B3 —
  it is reduced to diagnostic message text only, never a route to
  exemption (Correction 2; regression case 17). `legacy-work-ids.txt`
  continues its existing, unchanged purposes (gate-ordering checks that
  already run against a known frontmatter `work_id`) and is never
  consulted, directly or via filename inference, when frontmatter is
  entirely absent.
- Absence of frontmatter is never implicit legacy status — a file with no
  usable frontmatter must be on the *explicit* path grandfather list or it
  fails, unconditionally (case (C) in §3.3 Part 2).
- New governed specs/plans missing frontmatter fail closed — confirmed by
  regression tests #6–#8, #10, #16 (§5).
...
| 6 | New lowercase-named governed spec, no frontmatter | FAIL (`missing-governance-frontmatter`) | §3.3 Part 2 case (C): no frontmatter, path not on grandfather list (it's new) → unconditional fail regardless of casing |
| 7 | New uppercase-named governed spec, no frontmatter | FAIL | same mechanism — casing is irrelevant to the pass/fail branch |
| 8 | New governed spec/plan, no recognizable Work-ID token in filename, no frontmatter | FAIL, unless the written contract intentionally allows it | This design does **not** carve out an exception for tokenless new filenames — `GOVERNED_DOCUMENT_FRONTMATTER.md` already states every new spec/plan for a non-legacy work item must have frontmatter regardless of filename; §3.3 Part 2 case (C) enforces this unconditionally, closing exactly the class-1-shaped loophole a new (not historical) tokenless file would otherwise exploit |
| 9 | Explicitly grandfathered historical document, no frontmatter | PASS, only via the explicit historical mechanism | §3.3 Part 2 case (C), path found on `grandfathered-nonfrontmatter-documents.txt` |
| 10 | Non-grandfathered file that merely looks old / uses legacy-style naming | FAIL | Path not on the grandfather list → fail, regardless of superficial resemblance to a grandfathered file's naming convention; filename shape has zero bearing (Correction 2) |
| 11 | Correct frontmatter + correct Gate-2 reference | PASS (existing behavior preserved) | §3.3 Part 2 case (A), unchanged code path (lines 572–608) |
| 12 | Malformed/incomplete frontmatter | FAIL | §3.3 Part 2 case (B), unchanged `incomplete-governance-frontmatter` (lines 576–583) |
...
| 15 | Historical exact GAP-032 spec path, no frontmatter, exact path appears in grandfather list | PASS via **path grandfather only** | §3.3 Part 2 case (C); no filename regex or `legacy-work-ids.txt` lookup is involved in this pass — the exact path match alone is dispositive |
| 16 | A NEW non-grandfathered file whose filename appears to map to legacy GAP-032 (e.g. a lowercase/no-hyphen GAP032-shaped filename), no frontmatter | FAIL `missing-governance-frontmatter` | §3.3 Part 2 case (C); the file's exact path is not on the grandfather list, so it fails regardless of how legacy-like its filename looks — proves legacy-looking filename text cannot create exemption |
| 17 | Diagnostic regex, if retained: two otherwise identical non-grandfathered files — one whose filename contains a recognizable Work-ID token, one that does not | **SAME result: both FAIL** `missing-governance-frontmatter` | Filename-token regex, if present, affects only the violation message text (§3.3 Part 2, final paragraph); it has zero effect on the pass/fail branch, so both fixtures fail identically |
| 18 | Snapshot completeness: at the implementation head, every file in `docs/superpowers/specs/*.md` and `docs/superpowers/plans/*.md` either has complete frontmatter or its exact path occurs in the grandfather file | Zero `missing-governance-frontmatter` violations on a fresh bulk scan | Directly re-derives the non-frontmatter set at the implementation head and diffs it against the grandfather file's contents — this is what makes §3.4's "does not retroactively break `main`" claim a tested fact |
| 19 | Grandfather immutability: adding a new no-frontmatter spec/plan after the snapshot, without explicitly modifying the grandfather configuration | FAIL | Proves the grandfather file is a frozen historical snapshot, not a rolling allowlist — a new omission is never silently absorbed; the only way such a file can pass is (a) gaining real frontmatter, or (b) its own separate, reviewable governed edit adding it to the grandfather list |

The current 103 historical files (at the Gate 2 baseline) may remain
byte-unchanged; GAP-032/037/038/039/040/043/044 are not reopened.

### 5.1 Additional regression tests this design requires beyond the 14 Owner-specified cases

Cases 15–19 above (table) are the design-specific additions required
beyond the Owner's original 14, replacing the Round 1 draft's rejected
"legacy-ID-via-widened-regex ⇒ PASS" case (removed per Correction 2 — no
filename-derived pass path exists anywhere in the revised design, so no
test can exist to prove one works). Cases 18 and 19 in particular are what
make two claims tested facts rather than assertions:

- **Case 18 (snapshot completeness)** is what makes §3.4's "does not
  retroactively break `main`" claim a tested fact, not an assertion — a
  fresh bulk `--enforce-gate-ordering` run at the implementation head must
  produce zero `missing-governance-frontmatter` violations.
- **Case 19 (grandfather immutability)** is what makes §3.4's "frozen
  historical snapshot, not a rolling allowlist" claim a tested fact — it
  positively demonstrates that a new omission introduced after the
  snapshot is caught, not silently absorbed.
...
- The existing Correction-3 gate-ordering fixture tests (frontmatter
  presence/absence — likely `OwnerGovernanceLintFixtureTest.php` or
  equivalent; exact class name to be confirmed against the actual test
  suite at implementation time, not invented here) should be **extended**
  for cases 6, 7, 11, 12 (frontmatter presence/casing/malformedness) since
  they already own that exact surface.
- A **new, focused regression class** — e.g.
  `GateOrderingFrontmatterGrandfatherTest.php` — is warranted specifically
  for: the grandfather-path list mechanism and its exact-path-only
  semantics (cases 8, 9, 10, 15, 16), the diagnostic-regex/pass-fail
  equivalence proof (case 17), the snapshot-completeness test (case 18),
  and the grandfather-immutability test (case 19). This is new mechanism,
  not an extension of existing exemption or presence/absence logic, and
  deserves its own file so the grandfather-list contract has one
  canonical, directly-discoverable test home (mirroring why
  `GateOrderingDesignOnlyExemptionTest.php` exists as its own file rather
  than being folded into the general lint test).
...
generated data file — no permanent generator script is added under
`scripts/ssot/` (Correction 3; the grandfather file is produced by a
documented one-off command at implementation time, not committed tooling).
Reverting is a plain `git revert` of the implementation commit; no schema,
no database, no runtime state, no migration. `legacy-work-ids.txt` is not
modified, so no interaction with its own rollback story.
...
eliminated for the `docs/superpowers/{plans,specs}/*.md` corpus
specifically — every file with no usable frontmatter is now evaluated
against the single explicit grandfather-path list or fails; there is no
remaining "unrecognized filename → silently ignored" branch, and no
filename-derived pass path of any kind (Correction 2). Residual risk: a
new file could be *manually* (incorrectly) added to the grandfather-path
list by a future PR, since that list is a plain committed text file
rather than something structurally locked to a fixed historical commit.
This is the same residual trust model `legacy-work-ids.txt` already
carries today (a reviewer must notice an inappropriate addition) — not a
new or larger risk than the existing precedent, and mitigated the same
way: the file lives under `docs/owner-governance/**`, which forfeits the
...
case 19's regression test additionally proves such an addition is never
