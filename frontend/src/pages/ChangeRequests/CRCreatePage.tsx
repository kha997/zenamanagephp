    ...
    try {
      ...
      if (response.ok) {
        ...
        addNotification({
          type: 'success',
          title: 'Thành công',
          message: `Đã ${isDraft ? 'lưu nháp' : 'tạo'} yêu cầu thay đổi`
        });
      ...
      addNotification({
        type: 'error',
        title: 'Lỗi',
        message: 'Không thể tạo yêu cầu thay đổi'
      });
    } finally {
      setLoading(false);
    }
                  ...
                  {kpiEntries.map((entry, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <Input
                        ...
                        value={entry.key}
                        onChange={(e) => updateKpiEntry(index, 'key', e.target.value)}
                        className="flex-1"
                      />
                      <Input
                        placeholder="Giá trị (vd: +10%, -5%)"
                        value={entry.value}
                        onChange={(e) => updateKpiEntry(index, 'value', e.target.value)}
                        className="flex-1"
                      />
                      <Button
                        type="button"
