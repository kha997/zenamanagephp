...
export const useTemplatesStore = create<TemplatesState>()(devtools(
  (set, _get) => ({
    ...
    fetchTemplate: async (id: string) => {
      set({ loading: { isLoading: true, error: null } })
      try {
        const response = await api.get<ApiResponse<{ template: WorkTemplate }>>(`/work-templates/${id}`)
    ...
    createTemplate: async (data: CreateWorkTemplateForm) => {
      const response = await api.post<ApiResponse<{ template: WorkTemplate }>>('/work-templates', data)
    ...
    updateTemplate: async (id: string, data: Partial<CreateWorkTemplateForm>) => {
      const response = await api.put<ApiResponse<{ template: WorkTemplate }>>(`/work-templates/${id}`, data)
    ...
    duplicateTemplate: async (id: string, name: string) => {
      const response = await api.post<ApiResponse<{ template: WorkTemplate }>>(`/work-templates/${id}/duplicate`, { name })
    ...
    previewTemplateApplication: async (id: string, data: ApplyTemplateForm) => {
      const response = await api.post<ApiResponse<{ preview: TemplatePreview }>>(
    ...
    applyTemplate: async (id: string, data: ApplyTemplateForm) => {
      await api.post(`/work-templates/${id}/apply`, { ...data, preview_only: false })
    },

    getCategories: async () => {
      const response = await api.get<ApiResponse<{ categories: { value: string; label: string }[] }>>('/work-templates/meta/categories')
    ...
    getConditionalTags: async () => {
      const response = await api.get<ApiResponse<{ tags: string[] }>>('/work-templates/meta/conditional-tags')
