...
class SecurityRealtime {
    ...
    showToast(title, message, type = 'info') {
        ...
        toast.className = `fixed top-4 right-4 p-4 rounded-lg shadow-lg z-50 ${type === 'warning' ? 'bg-yellow-100 border-yellow-400 text-yellow-800' :
    ...
    updateConnectionStatus(status) {
        ...
        if (statusElement) {
            statusElement.className = `inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${status === 'connected' ? 'bg-green-100 text-green-800' :
