...
<div x-data="appSPA()" class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white shadow-sm border-b sticky top-20 z-40">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="border-t border-gray-200 py-3 hidden md:block">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-2">
                        ...
                        <!-- Analytics -->
                        <button @click="navigateTo('analytics')" class="app-nav-button flex items-center space-x-2 px-4 py-3 rounded-lg text-sm font-semibold" :class="currentView === 'analytics' ? 'active text-white' : 'text-gray-600 hover:text-gray-900'">
                            <i class="fas fa-chart-bar"></i>
                            <span>Analytics</span>
                        </button>
                        
                        <!-- Files -->
                        <button @click="navigateTo('files')" class="app-nav-button flex items-center space-x-2 px-4 py-3 rounded-lg text-sm font-semibold" :class="currentView === 'files' ? 'active text-white' : 'text-gray-600 hover:text-gray-900'">
            ...
            <nav class="border-t border-gray-200 py-2 md:hidden">
                <div class="flex items-center justify-between">
                    <!-- Mobile Menu Button -->
                    ...
                    <!-- Mobile Page Title -->
                    <h2 class="text-sm font-semibold text-gray-900" x-text="getPageTitle()"></h2>
                    
                    <!-- Mobile Refresh Button -->
                    <button @click="refreshData()" 
                            :disabled="refreshing"
                            class="flex items-center space-x-1 px-2 py-1 text-sm text-gray-600 hover:text-gray-900 disabled:opacity-50">
                        <i class="fas fa-sync-alt" :class="refreshing ? 'animate-spin' : ''"></i>
                    </button>
                </div>
            </nav>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-8">
        ...
        <div class="mb-6 md:mb-8">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900" x-text="getPageTitle()"></h1>
            <p class="mt-2 text-sm md:text-base text-gray-600" x-text="getPageDescription()"></p>
        </div>
        ...
        <div x-show="currentView === 'analytics'" x-transition>
            @include('app.analytics-content')
        </div>
        
        <!-- Files View -->
        <div x-show="currentView === 'files'" x-transition>
            @include('app.files-content')
        </div>
    </main>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('appSPA', () => ({
            ...
            async performGlobalSearch(query) {
                ...
                try {
                    const response = await fetch('/api/v1/app/search', {
                        method: 'POST',
                        headers: {
                    ...
                    if (data.success) {
                        this.handleSearchResults(data.data);
                    } else {
            ...
            handleSearchResults(results) {
                if (results.projects && results.projects.length > 0) {
                    ...
                    // Filter projects view to show search results
                } else if (results.tasks && results.tasks.length > 0) {
                    this.navigateTo('tasks');
                    // Filter tasks view to show search results
                } else if (results.documents && results.documents.length > 0) {
                    this.navigateTo('documents');
                    // Filter documents view to show search results
                } else if (results.team && results.team.length > 0) {
                    this.navigateTo('team');
                    // Filter team view to show search results
                }
            },
            ...
            refreshData() {
                ...
                switch(this.currentView) {
                    case 'dashboard':
                        ...
                        break;
                    case 'projects':
                        if (this.loadProjects) this.loadProjects();
                        break;
                    case 'tasks':
                        if (this.loadTasks) this.loadTasks();
                        break;
                    case 'calendar':
                        if (this.loadCalendar) this.loadCalendar();
                        break;
                    case 'documents':
                        if (this.loadDocuments) this.loadDocuments();
                        break;
                    case 'templates':
                        if (this.loadTemplates) this.loadTemplates();
                        break;
                    case 'team':
                        if (this.loadTeam) this.loadTeam();
                        break;
                    default:
                        this.loadDashboardData();
                }
            ...
            getPageDescription() {
                const descriptions = {
                    ...
                    'files': 'Upload, organize, and manage your files'
                };
        ...
        Alpine.data('templatesPage', () => ({
                    ...
                    this.templates = [
                        {
                            ...
                            description: 'Complete template for software projects',
                            category: 'software',
                            tasks: 15,
                            phases: 5,
                        ...
                        },
                        {
                            ...
                            description: 'Template for marketing projects',
                            category: 'marketing',
                            tasks: 8,
                            phases: 3,
                        ...
                        },
                        {
                            id: 3,
                            name: 'Event Planning',
                            description: 'Template for event management',
                            category: 'events',
                            tasks: 12,
                            phases: 4,
            ...
            createTemplate() {
                ...
                alert('Template creation will be implemented soon');
            }
        }));
        ...
        Alpine.data('teamPage', () => ({
            ...
            inviteMember() {
                ...
                alert('Member invitation will be implemented soon');
            },
            
            viewMember(memberId) {
                console.log('Viewing member:', memberId);
                // Implement member view logic
                alert('Member details will be implemented soon');
            },
            
            editMember(memberId) {
                console.log('Editing member:', memberId);
                // Implement member edit logic
                alert('Member editing will be implemented soon');
            }
        }));
