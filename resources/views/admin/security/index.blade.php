...
@section('title', 'Security Center - ZenaManage')

@section('styles')
<link rel="stylesheet" href="{{ asset('css/security-refresh.css') }}">
@endsection
...
<div class="container mx-auto p-6" x-data="securityPage()">
    ...
    <!-- Search & Filters -->
    @include('admin.security._filters')
    ...
    <!-- Modals -->
    @include('admin.security._modals.force_mfa_modal')
    @include('admin.security._modals.revoke_key_modal')
    @include('admin.security._modals.block_ip_modal')
    @include('admin.security._modals.end_session_modal')
    @include('admin.security._modals.edit_policy_modal')
</div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('securityPage', () => ({
        ...
        async loadCharts() {
            try {
                const response = await fetch(`/api/admin/security/charts-bypass?period=${this.chartPeriod}`, {
        ...
        createCharts() {
            try {
                ...
                // Create MFA chart
                if (this.chartData.mfaAdoption) {
                    const el = document.getElementById('mfa-adoption-chart');
                    if (el) {
                        this.charts.mfa = new Chart(el, {
                            type: 'line',
                            data: {
                ...
                // Create Successful Logins chart
                if (this.chartData.successfulLogins) {
                    const el = document.getElementById('successful-logins-chart');
                    if (el) {
                        this.charts.successful = new Chart(el, {
                            type: 'line',
                            data: {
                ...
                if (this.chartData.activeSessions) {
                    const el = document.getElementById('active-sessions-chart');
                    if (el) {
                        this.charts.sessions = new Chart(el, {
                            type: 'line',
                            data: {
                ...
                if (this.chartData.failedLogins) {
                    const el = document.getElementById('failed-logins-chart');
                    if (el) {
                        this.charts.failed = new Chart(el, {
                            type: 'line',
                            data: {
...
<script src="{{ asset('js/pages/dashboard-refresh.js') }}"></script>
<script>
