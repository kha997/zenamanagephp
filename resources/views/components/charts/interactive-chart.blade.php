{{--
    ...
    Usage:
    @include('components.interactive-chart', [
...
<div class="bg-white rounded-lg shadow-lg p-6" 
     x-data="interactiveChart('{{ $chartId }}', '{{ $type }}', @js($data))"
     x-init="init()">
    
    <!-- Chart Header -->
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold text-gray-900" x-text="title">{{ $title }}</h3>
    ...
    <!-- Chart Container -->
    <div :id="chartId" class="w-full" :style="'height: {{ $height }}px;'"></div>
...
function interactiveChart(chartId, type, data) {
    return {
        ...
        error: null,
        canDrillDown: true,
        realTimeEnabled: {{ $realTimeEnabled ? 'true' : 'false' }},
        ...
        initChart() {
            ...
            if ({{ $drillDownEnabled ? 'true' : 'false' }}) {
