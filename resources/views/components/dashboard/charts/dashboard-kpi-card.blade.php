...
<div class="kpi-card bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-all duration-300" 
    ...
    <div class="kpi-header flex items-center mb-4">
        <div class="kpi-icon w-12 h-12 {{ $icon_bg }} rounded-lg flex items-center justify-center mr-4">
            <i class="{{ $icon }} {{ $icon_text }} text-xl" aria-hidden="true"></i>
        </div>
        <div class="kpi-info flex-1">
            <h3 class="kpi-label text-sm font-medium text-gray-600 mb-1">{{ $label }}</h3>
            <p class="kpi-value text-2xl font-bold text-gray-900 kpi--{{ $kpi_key }}" 
               aria-label="{{ $label }} value">
                {{ $value }}
            </p>
            <p class="kpi-trend text-xs {{ $trend_class }}" 
    ...
    <div class="kpi-actions flex items-center space-x-2">
        @if($primary_action)
            <a href="{{ $primary_action['url'] }}" 
               ...
               data-action="primary"
               aria-label="{{ $primary_action['label'] }} for {{ $label }}">
        ...
        @if($secondary_action)
            <a href="{{ $secondary_action['url'] }}" 
               ...
               aria-label="{{ $secondary_action['label'] }} for {{ $label }}">
