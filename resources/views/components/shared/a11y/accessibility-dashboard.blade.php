...
{{-- Provides accessibility management interface --}}
...
document.addEventListener('alpine:init', () => {
    Alpine.data('accessibilityDashboard', () => ({
        ...
        async loadUserPreferences() {
            try {
                const response = await fetch('{{ route('api.accessibility.preferences') }}');
        ...
        async loadComplianceReport() {
            try {
                const response = await fetch('{{ route('api.accessibility.compliance-report') }}');
                const data = await response.json();
                if (data.success) {
        ...
        async loadStatistics() {
            try {
                const response = await fetch('{{ route('api.accessibility.statistics') }}');
                const data = await response.json();
                if (data.success) {
        ...
        async checkContrast() {
            try {
                const response = await fetch('{{ route('api.accessibility.check-color-contrast') }}', {
                    method: 'POST',
                    headers: {
        ...
        async exportReport() {
            try {
                const response = await fetch('{{ route('api.accessibility.generate-report') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
