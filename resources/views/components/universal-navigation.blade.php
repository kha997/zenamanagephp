...
{{-- Role-aware global navigation with active states and badges --}}
        ...
        <div class="flex items-center justify-between h-full">
            ...
            <div class="flex items-center space-x-1">
                @if(Auth::user()->hasRole('super_admin'))
                    {{-- Admin Navigation --}}
                    @include('components.navigation.admin-nav')
                @else
                    {{-- Tenant Navigation --}}
                    @include('components.navigation.tenant-nav')
                @endif
            </div>
    ...
    <div x-show="mobileMenuOpen" 
        ...
        <div class="px-4 py-2 space-y-1">
            @if(Auth::user()->hasRole('super_admin'))
                ...
                @include('components.navigation.mobile-admin-nav')
            @else
                {{-- Mobile Tenant Navigation --}}
                @include('components.navigation.mobile-tenant-nav')
            @endif
        </div>
    </div>
</nav>
