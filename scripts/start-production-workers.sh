...
for i in $(seq 1 $WORKERS_PER_QUEUE); do
    nohup php artisan queue:work redis --queue=emails-high --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS > "$LOG_DIR/worker-emails-high-$i.log" 2>&1 &
    WORKER_PID=$!
...
for i in $(seq 1 $WORKERS_PER_QUEUE); do
    nohup php artisan queue:work redis --queue=emails-medium --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS > "$LOG_DIR/worker-emails-medium-$i.log" 2>&1 &
    WORKER_PID=$!
...
for i in $(seq 1 $WORKERS_PER_QUEUE); do
    nohup php artisan queue:work redis --queue=emails-low --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS > "$LOG_DIR/worker-emails-low-$i.log" 2>&1 &
    WORKER_PID=$!
...
for i in $(seq 1 $WORKERS_PER_QUEUE); do
    nohup php artisan queue:work redis --queue=emails-welcome --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS > "$LOG_DIR/worker-emails-welcome-$i.log" 2>&1 &
    WORKER_PID=$!
...
command=php $PROJECT_PATH/artisan queue:work redis --queue=emails-high --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS
autostart=true
autorestart=true
stopasgroup=true
killasgroup=true
...
process_name=%(program_name)s_%(process_num)02d
command=php $PROJECT_PATH/artisan queue:work redis --queue=emails-medium --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS
autostart=true
autorestart=true
stopasgroup=true
killasgroup=true
...
process_name=%(program_name)s_%(process_num)02d
command=php $PROJECT_PATH/artisan queue:work redis --queue=emails-low --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS
autostart=true
autorestart=true
stopasgroup=true
killasgroup=true
...
process_name=%(program_name)s_%(process_num)02d
command=php $PROJECT_PATH/artisan queue:work redis --queue=emails-welcome --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS
autostart=true
autorestart=true
stopasgroup=true
killasgroup=true
