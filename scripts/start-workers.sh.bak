...
for i in $(seq 1 $WORKERS_PER_QUEUE); do
    nohup php artisan queue:work database --queue=emails-high --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS > storage/logs/worker-emails-high-$i.log 2>&1 &
    WORKER_PID=$!
...
for i in $(seq 1 $WORKERS_PER_QUEUE); do
    nohup php artisan queue:work database --queue=emails-medium --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS > storage/logs/worker-emails-medium-$i.log 2>&1 &
    WORKER_PID=$!
...
for i in $(seq 1 $WORKERS_PER_QUEUE); do
    nohup php artisan queue:work database --queue=emails-low --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS > storage/logs/worker-emails-low-$i.log 2>&1 &
    WORKER_PID=$!
...
for i in $(seq 1 $WORKERS_PER_QUEUE); do
    nohup php artisan queue:work database --queue=emails-welcome --sleep=$SLEEP --tries=$TRIES --max-time=$MAX_TIME --max-jobs=$MAX_JOBS > storage/logs/worker-emails-welcome-$i.log 2>&1 &
    WORKER_PID=$!
