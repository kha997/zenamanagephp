...
window.taskEditData = {
    ...
    newTag: '',
    formData: {
        id: <?php echo json_encode($task->id ?? ''); ?>,
        name: <?php echo json_encode($task->name ?? ''); ?>,
        description: <?php echo json_encode($task->description ?? ''); ?>,
        project_id: <?php echo json_encode($task->project_id ?? ''); ?>,
        assignee_id: <?php echo json_encode($task->assignee_id ?? ''); ?>,
        status: <?php echo json_encode($task->status ?? 'pending'); ?>,
        priority: <?php echo json_encode($task->priority ?? 'medium'); ?>,
        start_date: <?php echo json_encode($task->start_date ? \Carbon\Carbon::parse($task->start_date)->format('Y-m-d') : ''); ?>,
        end_date: <?php echo json_encode($task->end_date ? \Carbon\Carbon::parse($task->end_date)->format('Y-m-d') : ''); ?>,
        progress_percent: <?php echo e($task->progress_percent ?? '0'); ?>,
        estimated_hours: <?php echo e($task->estimated_hours ?? '0'); ?>,
        tags: <?php echo json_encode(array_filter(explode(',', $task->tags ?? ''))); ?>
    ...
    init() {
        ...
        console.log('Alpine.js initialized!');
        console.log('Raw task data from server:');
        console.log('Task ID:', <?php echo json_encode($task->id ?? 'NO_ID'); ?>);
        console.log('Task Name:', <?php echo json_encode($task->name ?? 'NO_NAME'); ?>);
        console.log('Task Status:', <?php echo json_encode($task->status ?? 'NO_STATUS'); ?>);
        console.log('Task Priority:', <?php echo json_encode($task->priority ?? 'NO_PRIORITY'); ?>);
        console.log('Task Assignee ID:', <?php echo json_encode($task->assignee_id ?? 'NO_ASSIGNEE'); ?>);
        console.log('Task Description:', <?php echo json_encode($task->description ?? 'NO_DESCRIPTION'); ?>);
        console.log('Task Project ID:', <?php echo json_encode($task->project_id ?? 'NO_PROJECT'); ?>);
        console.log('Task Start Date:', <?php echo json_encode($task->start_date ?? 'NO_START_DATE'); ?>);
        console.log('Task End Date:', <?php echo json_encode($task->end_date ?? 'NO_END_DATE'); ?>);
        console.log('Task Progress:', <?php echo json_encode($task->progress_percent ?? 'NO_PROGRESS'); ?>);
        console.log('Task Estimated Hours:', <?php echo json_encode($task->estimated_hours ?? 'NO_HOURS'); ?>);
        console.log('Task Tags:', <?php echo json_encode($task->tags ?? 'NO_TAGS'); ?>);
