<?php declare(strict_types=1);

namespace Tests\Feature\Feature\Admin;

use App\Models\User;
use App\Models\Project;
use App\Models\Tenant;
use App\Models\Role;
use App\Models\Permission;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\TestCase;

class AdminProjectsTest extends TestCase
{
    use RefreshDatabase;

    protected Tenant $tenant;
    protected Tenant $otherTenant;
    protected User $superAdmin;
    protected User $orgAdmin;
    protected User $regularUser;
    protected Project $project;
    protected Project $otherProject;

    protected function setUp(): void
    {
        parent::setUp();
        
        $this->tenant = Tenant::factory()->create();
        $this->otherTenant = Tenant::factory()->create();
        
        // Create permissions and roles
        $adminAccessPerm = Permission::create([
            'code' => 'admin.access',
            'module' => 'admin',
            'action' => 'access',
            'description' => 'Super Admin access',
        ]);
        
        $adminAccessTenantPerm = Permission::create([
            'code' => 'admin.access.tenant',
            'module' => 'admin',
            'action' => 'access.tenant',
            'description' => 'Org Admin access',
        ]);
        
        $projectsReadPerm = Permission::create([
            'code' => 'admin.projects.read',
            'module' => 'admin',
            'action' => 'projects.read',
            'description' => 'Read projects',
        ]);
        
        $projectsForceOpsPerm = Permission::create([
            'code' => 'admin.projects.force_ops',
            'module' => 'admin',
            'action' => 'projects.force_ops',
            'description' => 'Force operations',
        ]);
        
        $superAdminRole = Role::create([
            'name' => 'super_admin',
            'scope' => 'system',
            'is_active' => true,
        ]);
        $superAdminRole->permissions()->attach([$adminAccessPerm->id]);
        
        $orgAdminRole = Role::create([
            'name' => 'org_admin',
            'scope' => 'system',
            'is_active' => true,
        ]);
        $orgAdminRole->permissions()->attach([
            $adminAccessTenantPerm->id,
            $projectsReadPerm->id,
            $projectsForceOpsPerm->id,
        ]);
        
        $this->superAdmin = User::factory()->create();
        $this->superAdmin->roles()->attach($superAdminRole);
        
        $this->orgAdmin = User::factory()->create([
            'tenant_id' => $this->tenant->id,
        ]);
        $this->orgAdmin->roles()->attach($orgAdminRole);
        
        $this->regularUser = User::factory()->create([
            'tenant_id' => $this->tenant->id,
        ]);
        
        $this->project = Project::factory()->create([
            'tenant_id' => $this->tenant->id,
        ]);
        
        $this->otherProject = Project::factory()->create([
            'tenant_id' => $this->otherTenant->id,
        ]);
    }

    public function test_super_admin_can_access_projects_index(): void
    {
        $this->actingAs($this->superAdmin);
        
        $response = $this->get('/admin/projects');
        
        $response->assertStatus(200);
    }

    public function test_org_admin_can_access_projects_index(): void
    {
        $this->actingAs($this->orgAdmin);
        
        $response = $this->get('/admin/projects');
        
        $response->assertStatus(200);
    }

    public function test_regular_user_cannot_access_projects_index(): void
    {
        $this->actingAs($this->regularUser);
        
        $response = $this->get('/admin/projects');
        
        $response->assertStatus(403);
    }

    public function test_super_admin_sees_all_projects(): void
    {
        $this->actingAs($this->superAdmin);
        
        $response = $this->get('/admin/projects');
        
        $response->assertStatus(200);
        $response->assertSee($this->project->name);
        $response->assertSee($this->otherProject->name);
    }

    public function test_org_admin_sees_only_own_tenant_projects(): void
    {
        $this->actingAs($this->orgAdmin);
        
        $response = $this->get('/admin/projects');
        
        $response->assertStatus(200);
        $response->assertSee($this->project->name);
        $response->assertDontSee($this->otherProject->name);
    }

    public function test_super_admin_can_freeze_project(): void
    {
        $this->actingAs($this->superAdmin);
        
        $response = $this->postJson("/admin/projects/{$this->project->id}/freeze", [
            'reason' => 'Test freeze',
        ]);
        
        $response->assertStatus(200);
        $response->assertJson(['success' => true]);
        
        $this->project->refresh();
        $this->assertEquals('frozen', $this->project->status);
    }

    public function test_org_admin_can_freeze_own_tenant_project(): void
    {
        $this->actingAs($this->orgAdmin);
        
        $response = $this->postJson("/admin/projects/{$this->project->id}/freeze", [
            'reason' => 'Test freeze',
        ]);
        
        $response->assertStatus(200);
        $response->assertJson(['success' => true]);
    }

    public function test_org_admin_cannot_freeze_other_tenant_project(): void
    {
        $this->actingAs($this->orgAdmin);
        
        $response = $this->postJson("/admin/projects/{$this->otherProject->id}/freeze", [
            'reason' => 'Test freeze',
        ]);
        
        $response->assertStatus(404); // Not found because of tenant scope
    }

    public function test_regular_user_cannot_freeze_project(): void
    {
        $this->actingAs($this->regularUser);
        
        $response = $this->postJson("/admin/projects/{$this->project->id}/freeze", [
            'reason' => 'Test freeze',
        ]);
        
        $response->assertStatus(403);
    }
}
