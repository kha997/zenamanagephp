...
function buildArtisanEnv(): NodeJS.ProcessEnv {
  ...
  // Round 158: Default to MySQL to match webServer command configuration
  // Round 161: Use zenamanage_e2e as default to match webServer
  // This ensures consistency between global-setup and webServer
  if (!mergedEnvVars.DB_CONNECTION && !process.env.DB_CONNECTION) {
    console.log('⚠️  No DB_CONNECTION found, defaulting to MySQL for E2E tests (Round 158)');
    overrides.DB_CONNECTION = 'mysql';
    overrides.DB_HOST = process.env.DB_HOST || '127.0.0.1';
    overrides.DB_PORT = process.env.DB_PORT || '3306';
    overrides.DB_DATABASE = process.env.DB_DATABASE || 'zenamanage_e2e';
    overrides.DB_USERNAME = process.env.DB_USERNAME || 'root';
    overrides.DB_PASSWORD = process.env.DB_PASSWORD || '';
  } else {
    ...
    if (mergedEnvVars.DB_CONNECTION) {
      ...
      // Round 161: Set default DB_DATABASE for MySQL to match webServer
      if (mergedEnvVars.DB_CONNECTION === 'mysql' && !mergedEnvVars.DB_DATABASE && !process.env.DB_DATABASE) {
        overrides.DB_DATABASE = 'zenamanage_e2e';
      } else if (mergedEnvVars.DB_DATABASE) {
      ...
      // Round 161: Set default DB_DATABASE for MySQL to match webServer
      if (process.env.DB_CONNECTION === 'mysql' && !process.env.DB_DATABASE && !mergedEnvVars.DB_DATABASE) {
        overrides.DB_DATABASE = 'zenamanage_e2e';
      } else if (process.env.DB_DATABASE) {
        overrides.DB_DATABASE = process.env.DB_DATABASE;
      }
